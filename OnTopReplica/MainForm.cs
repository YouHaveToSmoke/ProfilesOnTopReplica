...
namespace OnTopReplica {
    ...
    partial class MainForm : AspectRatioForm {
        ...
        /// <summary>
        /// Gets whether the form is currently showing a thumbnail.
        /// </summary>
        public bool IsShowingThumbnail {
            get {
                return _thumbnailPanel != null && _thumbnailPanel.IsShowingThumbnail;
            }
        }

        #endregion

        #region Clone window functionality

        /// <summary>
        /// Clones the current window by opening a new MainForm instance.
        /// </summary>
        public void CloneCurrentWindow() {
            if (!IsShowingThumbnail || CurrentThumbnailWindowHandle == null) {
                MessageBox.Show(
                    "Kein Fenster ausgewählt. Bitte wählen Sie zuerst ein Fenster zum Klonen aus.",
                    "Kein Fenster",
            ...
            try {
                ...
                if (newLocation.X + this.Width > screen.WorkingArea.Right) {
                    newLocation.X = screen.WorkingArea.Right - this.Width;
                }
                if (newLocation.Y + this.Height > screen.WorkingArea.Bottom) {
                    newLocation.Y = screen.WorkingArea.Bottom - this.Height;
                }
                ...
                // Apply current settings to new form after it's shown
                var currentHandle = CurrentThumbnailWindowHandle;
                var currentRegion = SelectedThumbnailRegion;

                newForm.Shown += (sender, e) => {
                    try {
                        // Set the same thumbnail
                        newForm.SetThumbnail(currentHandle, currentRegion);

                        // Copy visual settings
                        newForm.Opacity = this.Opacity;
                        newForm.ClickThroughEnabled = this.ClickThroughEnabled;
                        newForm.ClickForwardingEnabled = this.ClickForwardingEnabled;
                        newForm.IsChromeVisible = this.IsChromeVisible;
                        newForm.TopMost = this.TopMost;
                    }
                    catch (Exception ex) {
                        Log.WriteException("Error setting thumbnail on cloned window", ex);
                        MessageBox.Show(
                            $"Fehler beim Klonen des Fensters:\n{ex.Message}",
                            "Fehler",
                            MessageBoxButtons.OK,
                ...
                Log.Write($"Cloned window to new instance at {newLocation}");
            }
            catch (Exception ex) {
                Log.WriteException("Error creating cloned window", ex);
                MessageBox.Show(
                    $"Fehler beim Erstellen des neuen Fensters:\n{ex.Message}",
                    "Fehler",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error
                );
